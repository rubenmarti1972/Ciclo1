...
def indicaciones_users(zona: tuple,ubicacion_actual:tuple):
    ...
    #Almacena variables 
    informacion_recorrido["zonawifi1"] = [lat, lon, cant_personas]
...
def zona_wificerc():
    ...
    if len(coordenadas_usuario) > 0:
        ...
        if (opcion >=0) and (opcion<=3): 
            ...
            informacion_recorrido["actual"] = punto_seleccionado
            distancias:list= []
...
def guardar_archivo():
    ...
    if len(coordenadas_usuario) == 0 or len(informacion_recorrido) == 0:
...
def actualizar_zonas_wifi():
    try:
        global ubicacion_users
        archivo = open("coordenadas.txt")
        ubicacion_users = eval(archivo.read())        
